...
class MOAgent(ABC):
    ...
    def extract_env_info(self, env: Optional[gym.Env]) -> None:
        ...
        if env is not None:
            ...
            # Check if observation_space is a Dict
            if isinstance(self.env.observation_space, spaces.Dict):
                # Initialize observation_shape as a dict to hold each observation element's shape
                self.observation_shape = {key: space.shape for key, space in self.env.observation_space.spaces.items()}
                # Sum the product of each observation's dimensions to get the total observation dimension
                self.observation_dim = sum(np.prod(space.shape) for space in self.env.observation_space.spaces.values())
            ...
            # Process the action space as before
